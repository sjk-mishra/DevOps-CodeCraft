...
pip install Flask
Navigate to .env and set the environment variables. It requires the server credentials on which the project will run or you wish to creat the resources and the resource details.

Start the Flask application by running the following command from the project root directory:

python app.py
Access the Flask application in your web browser at http://localhost:5000 and interact with the provided routes to trigger Terraform scripts.
